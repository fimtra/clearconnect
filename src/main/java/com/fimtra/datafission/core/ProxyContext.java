...
/**
 * A proxy context allows a local runtime to observe records from a single {@link Context} in a remote
 * runtime.
 * <p>
 * A proxy context connects to a single remote {@link Publisher} instance using a 'transport channel' ({@link
 * ITransportChannel}). On construction of the proxy context, the channel is established. If the channel is
 * interrupted, the proxy context re-initialises the channel and re-subscribes for all currently subscribed
 * records being observed. This will continue indefinitely until the channel is made or the {@link #destroy()}
 * method is called.
 * <p>
 ...
 * Subscribing for the {@link #RECORD_CONNECTION_STATUS_NAME} record allows application code to detect when
 * records are stale (effectively connection to the remote has been lost). If application code needs to
 * 'clear' records when they are disconnected in anticipation for re-population on connection, call the {@link
 * #resubscribe(String...)} method.
 * <p>
 * A proxy context receives changes to records from a {@link Publisher}. The proxy context requests the
 * records that should be observed by the publisher for changes. There is a 1:n relationship between a
 * publisher and a proxy context; one publisher can be publishing to many proxy contexts.
 * <p>
 * When a remote record is subscribed for, a local 'proxy' is created that reflects all changes from the
 * remote. When there are no more observers of the remote record, this local proxy record is deleted.
 * <p>
 * A proxy context has a special record; a remote context registry which shows the context registry of the
 * remote context. Use this to find out what records exist in the remote context. Add an observer for the
 * {@link IRemoteSystemRecordNames#REMOTE_CONTEXT_RECORDS} record to get the context registry of the remote
 * context.
 * <p>
 * A proxy context has another special record; a remote connection status record. This shows the connection
 * status on a per record basis. Subscribing for this record ( {@link #RECORD_CONNECTION_STATUS_NAME})
 * provides application code with the ability to detect connection status changes for a record.
 *
...
public final class ProxyContext implements IObserverContext {
    ...
    /**
     * Encapsulates all the remote system records. These are effectively the system records in a remote
     * context.
     *
     * @author Ramon Servadei
     */
    public interface IRemoteSystemRecordNames {
        ...
        /**
         * This record shows the context record of the remote context. Use this to find out what records exist
        ...
        /**
         * This record shows the context subscriptions of the remote context. Use this to find out what
         * records are being subscribed for in the remote context.
         * <p>
         * Use {@link #getRecord(String)} to obtain the remote context subscriptions using this string.
         *
         ...
         * <p>
         * Use {@link #getRecord(String)} to obtain the available RPCs in the remote context using this
         * string.
         *
         ...
         * <p>
         * Use {@link #getRecord(String)} to obtain the remote context connections using this string.
         *
     ...
     * </pre>
     * <p>
     * A special record that holds the connection status per record subscribed from the remote context. The
     * keys are the record names, the value is a string indicating the connection status for the record, one
     * of: {@link #RECORD_CONNECTED} or {@link #RECORD_DISCONNECTED}.
     * <p>
     * There are occasions where a record will have a different connection status to other records from the
     * same remote context - typically during a record re-sync operation.
     *
     * @see ProxyContext#isRecordConnected(String)
     */
    public static final String RECORD_CONNECTION_STATUS_NAME = "RecordConnectionStatus";
    /**
     * The value used to indicate a record is connected in the {@link #RECORD_CONNECTION_STATUS_NAME} record
     */
    public static final TextValue RECORD_CONNECTED = TextValue.valueOf("CONNECTED");
    /**
     * The value used to indicate a record is (re)connecting in the {@link #RECORD_CONNECTION_STATUS_NAME}
     * record
     */
    public static final TextValue RECORD_CONNECTING = TextValue.valueOf("CONNECTING");
    /**
     * The value used to indicate a record is disconnected in the {@link #RECORD_CONNECTION_STATUS_NAME}
     * record
     */
    ...
    static
    {
        ...
        mapping.put(IRemoteSystemRecordNames.REMOTE_CONTEXT_CONNECTIONS,
                ISystemRecordNames.CONTEXT_CONNECTIONS);
        mapping.put(IRemoteSystemRecordNames.REMOTE_CONTEXT_SUBSCRIPTIONS,
        ...
        mapping.put(ISystemRecordNames.CONTEXT_CONNECTIONS,
                IRemoteSystemRecordNames.REMOTE_CONTEXT_CONNECTIONS);
        mapping.put(ISystemRecordNames.CONTEXT_SUBSCRIPTIONS,
    ...
    private final class RpcResultHandler implements ISequentialRunnable {
        ...
        public void run()
        {
            if (log)
            {
                if (!logRx)
                {
                    Log.log(ProxyContext.this, "(<-) RPC result ",
    ...
    private static final class ProxyContextReceiver implements IReceiver {
                ...
                this.proxyContext.executeSequentialCoreTask(new ISequentialRunnable() {
                    ...
                    public void run()
                    {
                        try
                        {
                            ...
                            ContextUtils.clearNonSystemRecords(
                                    ProxyContextReceiver.this.proxyContext.context);
        ...
        public void onDataReceived(final ByteBuffer data, final ITransportChannel source)
        {
            ...
            if (this.proxyContext.channelToken == this.receiverToken)
            {
                this.proxyContext.executeSequentialCoreTask(
                        RX_FRAME_HANDLER_POOL.get().initialise(data, source, this));
            }
        }
    ...
    /**
     * Handles the received TCP frame data. If the data is an atomic change, this will be passed to a {@link
                ...
                catch (IncorrectSequenceException e)
                {
                    ...
                    if (this.proxyContext.resync(recordName))
                    {
                        Log.log(this.receiver, "Resync of " + recordName
                                + " started due to error processing received change", e);
                    }
                    else
                    {
                        Log.log(this.receiver, "Resync of ", recordName,
    ...
    /**
     * Handles a single {@link IRecordChange} created from a received frame decoded from a {@link
     * RxFrameHandler}
     *
     * @author Ramon Servadei
     */
    private final static class RxAtomicChangeHandler implements ISequentialRunnable {
        ...
        RxAtomicChangeHandler initialise(String changeName, IRecordChange changeToApply,
                ProxyContext proxyContext)
        {
            this.changeName = changeName;
            this.proxyContext = proxyContext;
        ...
        public void run()
        {
            try
            {
                ...
                final IRecordListener[] subscribers =
                        this.proxyContext.context.recordObservers.getSubscribersFor(name);
                if (!(subscribers.length > 0))
                {
                    Log.log(this.proxyContext,
                            "Received record but no subscription exists - ignoring record=", this.changeName,
                            " seq=", (isImage ? "i" : "d"), Long.toString(this.changeToApply.getSequence()));
                    return;
                }
                ...
                {
                    switch(this.proxyContext.imageDeltaProcessor.processRxChange(this.changeToApply, name,
                            record))
                    {
                        case ImageDeltaChangeProcessor.PUBLISH:
                            ...
                            // on top of it
                            this.proxyContext.context.setSequence((Record) record, record.getSequence());
            ...
            {
                Log.log(this.proxyContext,
                        "Could not process received message " + ObjectUtils.safeToString(this.changeToApply),
                        e);
            }
            finally
            {
    ...
    /**
     * The period in milliseconds to wait before trying a reconnect, default is {@link
     * Values#PROXY_CONTEXT_RECONNECT_PERIOD_MILLIS}
     */
    int reconnectPeriodMillis = DataFissionProperties.Values.PROXY_CONTEXT_RECONNECT_PERIOD_MILLIS;
    /**
     * A flag that allows receivers to know whether their channel is still valid for the context; when a
     * socket disconnects, the receiver is still attached to the socket and the proxy - we don't want this
     * receiver to invoke events on the proxy as they are no longer valid.
     */
    volatile Object channelToken;
    /**
     * Tracks responses to subscription actions so that a subscription is known to have been processed by the
     * receiver.
     */
    ...
    /**
     * Keeps an instance of each RPC that is requested so it can be used for cloning on any subsequent
     * requests for the same RPC.
     */
    final Map<String, RpcInstance> rpcTemplates;

    /**
     * Construct the proxy context and connect it to a {@link Publisher} using the specified host and port.
     * <p>
     * This uses the transport technology defined by {@link TransportTechnologyEnum#getDefaultFromSystemProperty()}
     *
     * @param name          the name for this proxy context - this is used by the remote context to identify
     *                      this proxy
     * @param codec         the codec to use for sending/receiving messages from the {@link Publisher}
     * @param publisherNode the end-point node of the publisher process
     * @param publisherPort the end-point port of the publisher process
     */
    public ProxyContext(String name, ICodec codec, final String publisherNode, final int publisherPort)
    {
        this(name, codec, publisherNode, publisherPort,
                TransportTechnologyEnum.getDefaultFromSystemProperty(), name);
    }

    /**
     * Construct the proxy context and connect it to a {@link Publisher} using the specified host, port and
     * transport technology
     *
     * @param name                the name for this proxy context - this is used by the remote context to
     *                            identify this proxy
     * @param codec               the codec to use for sending/receiving messages from the {@link Publisher}
     * @param publisherNode       the end-point node of the publisher process
     * @param publisherPort       the end-point port of the publisher process
     * @param transportTechnology the transport technology to use
     * @param sessionContextName  the name given to the "session" between this proxy and its remote context.
     */
    public ProxyContext(String name, ICodec codec, final String publisherNode, final int publisherPort,
            ...
            String sessionContextName)
    {
        ...
        this.sessionCache = new NotifyingCache<ISessionListener, Pair<String, Boolean>>() {
            @Override
            protected void notifyListenerDataRemoved(ISessionListener listener, String key,
            ...
            @Override
            protected void notifyListenerDataAdded(ISessionListener listener, String key,
                    Pair<String, Boolean> data)
            {
     ...
     * <p>
     * Note that the connection may be broken, use {@link #isConnected()} to determine this. Be aware that
     * there may be a race condition if calling {@link #isConnected()} on a channel whilst the connection is
     * happening, so just because {@link #isConnected()} returns <code>false</code> does not necessarily mean
     * that the end point address is not available. Only when {@link #isConnected()} returns <code>true</code>
     * can this end point address be guaranteed.
     */
    ...
    /**
     * Set the period to wait before attempting to reconnect after the connection has been unexpectedly
     * broken.
     *
     * @param reconnectPeriodMillis the period in milliseconds to wait before trying a reconnect to the
     *                              context, cannot be less than 50
     * @see Values#PROXY_CONTEXT_RECONNECT_PERIOD_MILLIS
     */
    ...
    /**
     * This reconnects the proxy context to the new publisher - used when a publisher changes. If the proxy
     * context is currently connected, calling this method will close the current connection and attempt to
     * open a connection to the new publisher.
     *
     * @param node the node (hostname) of the publisher
     * @param port the port (transport specific) for the publisher
     */
    public void reconnect(final String node, final int port)
    {
        setTransportChannelBuilderFactory(
                TransportChannelBuilderFactoryLoader.load(this.codec.getFrameEncodingFormat(),
     ...
     *
     * @param recordName    the record name to get
     * @param timeoutMillis the timeout in milliseconds for the operation
     * @return an immutable image of the record, <code>null</code> if the timeout expires or there is no
     * record
     */
            ...
            String... recordNames)
    {
        ...
        synchronized (this.lock)
        {
            ...
            final String[] recordsToSubscribeFor =
                    getEligibleRecords(this.context.recordObservers, recordNames);
    ...
    public CountDownLatch removeObserver(IRecordListener observer, String... recordNames)
    {
        ...
        synchronized (this.lock)
        {
            ...
            final String[] recordsToUnsubscribe =
                    getEligibleRecords(this.context.recordObservers, recordNames);
    ...
    public void destroy()
    {
        synchronized (this.lock)
        {
            if (this.active)
            {
                ...
                this.active = false;
                cancelReconnectTask();
                ContextExecutorFactory.remove(ProxyContext.this);
    ...
    ITransportChannel constructChannel() throws IOException
    {
        ...
        Log.log(this, "Constructing channel for ", getShortName(), " using ",
                ObjectUtils.safeToString(channelBuilder));
    ...
    {
        final IRecordChange changeToApply = this.teleportReceiver.combine(
                (AtomicChange) this.codec.getAtomicChangeFromRxMessage(frameHandler.data));
        ...
        if (logVerboseSubscribes && this.firstUpdateExpected.size() > 0)
        {
            synchronized (this.firstUpdateExpected)
            {
                if (this.firstUpdateExpected.size() > 0)
                {
                    ...
                    {
                        Log.log(ProxyContext.this, "(<-) first update from [",
                                this.channel.getEndPointDescription(), "] for [",
                                changeToApply.getName() + "]");
                    }
                }
            }
        }
        ...
        executeSequentialCoreTask(
                frameHandler.atomicChangeHandler.initialise(changeName, changeToApply, this));
    ...
    /**
     * Performs a data re-sync of the named record. A re-sync operation differs from a {@link
     * #resubscribe(String...)} in that a re-sync may not trigger a notification to observing {@link
     * IRecordListener} instances when the re-sync receives its image. A re-subscribe, on the other hand, will
     * always trigger a notification as it does a complete tear-down then re-subscribe.
     *
     * @param name the record to re-sync.
    ...
    boolean resync(final String name)
    {
        ...
        if (resyncNeeded)
        {
            ...
            finalEncodeAndSendToPublisher(
                    ProxyContext.this.codec.getTxMessageForResync(substituteRemoteNameWithLocalName(name)));
    ...
    {
        final IValue definition =
                this.context.getRecord(IRemoteSystemRecordNames.REMOTE_CONTEXT_RPCS).get(name);
        if (definition == null)
        {
            return null;
        }
    ...
    void setupReconnectTask()
    {
        synchronized (this.lock)
        {
            ...
            if (rpcRecord.size() > 0)
            {
                Log.log(this, "Removing RPCs ", ObjectUtils.safeToString(rpcRecord.keySet()), " from ",
                        getShortName());
                synchronized (this.context.getRecord(
                        IRemoteSystemRecordNames.REMOTE_CONTEXT_RPCS).getWriteLock())
                {
                    rpcRecord.clear();
            ...
            this.reconnectTask = ContextExecutorFactory.get(ProxyContext.this).schedule(
    ...
    void updateConnectionStatus(Connection connectionStatus)
    {
        ...
        synchronized (this.remoteConnectionStatusRecord.getWriteLock())
        {
            for (String recordName : recordNames)
            {
                ...
                {
                    Log.log(this, "Could not update record status " + recordName + " to "
                            + ObjectUtils.safeToString(status), e);
                }
            }
            this.context.publishAtomicChange(RECORD_CONNECTION_STATUS_NAME);
        }
    ...
    /**
     * @return a short-hand string describing both connection points of the {@link TcpChannel} of this proxy
     */
    ...
    void subscribe(final String permissionToken, final String[] recordsToSubscribeFor)
    {
        ...
        for (i = 0; i < size; i++)
        {
            ...
            {
                subscribeBatch(permissionToken, batchSubscribeRecordNames.toArray(new String[0]), i, size);
    ...
    private void subscribeBatch(final String permissionToken, final String[] recordsToSubscribeFor,
    ...
    void unsubscribe(final String[] recordsToUnsubscribe)
    {
        ...
        for (i = 0; i < size; i++)
        {
            ...
            {
                unsubscribeBatch(batchUnsubscribeRecordNames.toArray(new String[0]), i, size);
