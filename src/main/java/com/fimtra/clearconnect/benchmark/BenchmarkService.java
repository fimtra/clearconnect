...
/**
 * A service that benchmarks the platform and network environment. It requires other
 * {@link EchoService} instances to be running, publishes records and measures the latency for
...
public class BenchmarkService
{
    ...
    {
        this.agent = new PlatformRegistryAgent("benchmark-agent", registryHost);
        this.agent.createPlatformServiceInstance(BENCHMARK_SERVICE, "primary", registryHost, WIRE_PROTOCOL,
        ...
        results.append(SystemUtils.lineSeparator()).append("totalTimeSecs=").append(
            (long) (((double) timeMicros) / 1000000));
        results.append(SystemUtils.lineSeparator()).append("txMsgsPerSec=").append(
            (long) (((double) totalMessagesTx.get() / timeMicros) * 1000000));
        results.append(SystemUtils.lineSeparator()).append("rxMsgsPerSec=").append(
            (long) (((double) totalMessagesRx.get() / timeMicros) * 1000000));
        results.append(SystemUtils.lineSeparator()).append("networkMsgsPerSec=").append(
            (long) (((double) (totalMessagesRx.get() + totalMessagesTx.get()) / timeMicros) * 1000000));
        ...
        {
            kbPublished +=
                connectionsRecords.getOrCreateSubMap(key).get(IContextConnectionsRecordFields.KB_COUNT).longValue();
            msgsPublished +=
                connectionsRecords.getOrCreateSubMap(key).get(IContextConnectionsRecordFields.MESSAGE_COUNT).longValue();
        }
        ...
        results.append(SystemUtils.lineSeparator()).append("avgMsgSizeBytes=").append(
            (long) (((double) kbPublished / msgsPublished) * 1024));
        Log.banner(this, results.toString());
        System.err.println(results.toString());
        System.err.println(System.getProperty("os.name") + " (" + System.getProperty("os.version") + "), "
            + System.getProperty("os.arch") + ", Java " + System.getProperty("java.version") + ", cpus="
    ...
    long doEchoLatencyTest(Set<String> echoServiceNames, final int maxRecordCount, final int maxFieldCount,
        final int runCount) throws InterruptedException
    {
        ...
        final IRecordListener echoListener = new IRecordListener()
        {
            ...
            public void onChange(IRecord imageCopy, IRecordChange atomicChange)
            {
                ...
                if (signatureField != null
                    && is.eq(signatureField.textValue(), recordSignatures.get(imageCopy.getName())))
                {
        ...
        try
        {
            ...
            for (int recordCount = 0; recordCount < maxRecordCount; recordCount++)
            {
                ...
                {
                    this.agent.getPlatformServiceProxy(serviceName).addRecordListener(echoListener, recordName);
                }
            }
            ...
            for (int recordCount = 0; recordCount < maxRecordCount; recordCount++)
            {
                ...
                try
                {
                    ...
                    {
                        throw new IllegalStateException("Did not get subscription from all echo services for "
                            + recordName + "..." + subscribers.getCount());
                    }
                }
                finally
                {
            ...
            Log.log(this, "Waiting for " + totalUpdateCounter.getCount() + " echo responses from " + echoServiceCount
                + " echo services...");
        ...
        finally
        {
            for (int recordCount = 0; recordCount < maxRecordCount; recordCount++)
            {
                ...
                for (String serviceName : echoServiceNames)
                {
                    this.agent.getPlatformServiceProxy(serviceName).removeRecordListener(echoListener, recordName);
                }
            }
        }
    }
